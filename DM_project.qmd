...
First, some variables contain an important proportion of missing values (e.g. social contributions and hospital beds). The NA are primarily concentrated at the beginning of the time period studied (2000-2023) and likely reflect data reporting issues rather than an absence of social policies such as health contributions, usually marked as 0 in the rest of the dataset. 
...
### TO DO : what will the modelling deal with this ?

```{r}
# removing countries with too many NAs 
database <- database |>
  filter(!REF_AREA %in% c("ZAF", "ARG", "PER", "THA"))
