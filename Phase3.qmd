...
Tests using lm instead of lmer to keep mortality_lag1 and remove the random intercept 
```{r}
second_test <- lm(OBS_VALUE_mortality ~ mortality_lag1 + healthExp_lag1 + healthExp_ma5 + hospBeds_lag1 + hospBeds_ma5 + socialExp_lag1 + socialExp_ma5 + OBS_VALUE_gdp + OBS_VALUE_unemp, 
                  data = data_lagged)
summary(second_test)
```
...
```{r}
third_test <- lm(
  OBS_VALUE_mortality ~ mortality_lag1 +
    healthExp_ma5 + hospBeds_ma5 + socialExp_ma5 +
    OBS_VALUE_gdp + OBS_VALUE_unemp,
  data = data_lagged
)
summary(third_test)
```

Problems when using lm instead of lmer to keep mortality_lag1 : mortality_lag1 explains most of mortality (Since the mortality rate is a flux, it is logical that most of its current value is explained by its past value). As a result it is difficult to identify the effect of previous investment in health and social protection which have really small coefficients in comparison.   

It seems better to keep using lmer and remove mortality_lag1 in order to highlight the effects of health expenditure and social expenditure on mortality.
...
Considering our question, the most interesting results for us to look at are the medium-run results. Indeed what we are trying to grasp is how adopting a health or social policy (and therefore investing in health) can help a country reduce their avoidable mortality. 
The medium run model will be our main model. 

In spite of this choice it is still interesting to check how the results vary from one model to another to compare the different effects. 
...
First, even if it is not the most essential in our analysis, checking the results for time = t
 ...
 - reverse causality is likely, don't add into main results 
```{r}
