...
```{r}
# creating an average trend for avoidable mortality for all countries 
average_trend <- database |>
  group_by(TIME_PERIOD) |>
  summarise(mean_mortality = mean(OBS_VALUE_mortality, na.rm = TRUE))

# graph with mortality split by countries + adding average trend 
database |>
...
ggplot(database, aes(x = OBS_VALUE_social_expenditure, y = OBS_VALUE_mortality)) +
  geom_point(aes(color = REF_AREA)) +
  theme(legend.position = "none") + # deleting the legend as it the same for the 4 graphs
  labs(
...
# IV. Econometric Analysis : Linear Mixed Model (LMM)
...
```{r}
# creating a database with lagged variables 
...
After trying a first model, we notice 3 main problems and modify the following models accordingly.

1. The correlation between `mortality_lag1` and the intercept is very high, nullifying the effect of the random intercept (variance and standard deviation equal to 0). This shows that past mortality already explains most differences between countries. We decide to remove `mortality_lag1` from the next regressions.
...
```{r}
# rescaling the GDP per capita variable to unit per 1,000 USD
...
- 2 variables as random effects : social expenditure and health expenditure. This will allow us to better perceive the differences between countries when it comes to social and health policies.  
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HealthExp_{i,t-1}
+ \beta_{2}\, HospBeds_{i,t-1}
+ \beta_{3}\, SocialExp_{i,t-1} \\[0.4em]
&+ \beta_{4}\, GDP_{i,t}
+ \beta_{5}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-1}
+ u_{2i}\, SocialExp_{i,t-1}
+ \varepsilon_{i,t}
\end{aligned}
\end{equation*}
...
```{r}
model_short_run <- lmer(OBS_VALUE_mortality ~ healthExp_lag1 + 
                          hospBeds_lag1 + 
                          socialExp_lag1 + 
                          OBS_VALUE_gdp + 
                          OBS_VALUE_unemp + 
                          (1 | REF_AREA) + # random intercept by country 
                          (0 + healthExp_lag1 | REF_AREA) + # adding random slopes
                          (0 + socialExp_lag1 | REF_AREA), 
                        data = data_lagged, 
                        REML = FALSE) # used to get AIC and compare models
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HealthExp_{i,t-[1,3]}
+ \beta_{2}\, HospBeds_{i,t-[1,3]}
+ \beta_{3}\, SocialExp_{i,t-[1,3]} \\[0.4em]
&+ \beta_{4}\, GDP_{i,t}
+ \beta_{5}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-[1,3]}
+ u_{2i}\, SocialExp_{i,t-[1,3]}
+ \varepsilon_{i,t}
\end{aligned}
\end{equation*}
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HealthExp_{i,t-[1,5]}
+ \beta_{2}\, HospBeds_{i,t-[1,5]}
+ \beta_{3}\, SocialExp_{i,t-[1,5]} \\[0.4em]
&+ \beta_{4}\, GDP_{i,t}
+ \beta_{5}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-[1,5]}
+ u_{2i}\, SocialExp_{i,t-[1,5]}
+ \varepsilon_{i,t}
\end{aligned}
\end{equation*}
...
```{r}
# adding a pandemic dummy 
data_lagged <- data_lagged |>
  mutate(
    pandemic = ifelse(TIME_PERIOD %in% c(2020:2021), 1, 0) # 1 if data in 2020-2021, 0 otherwise 
  )
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HospBeds_{i,t-1}
+ \beta_{2}\, HealthExp_{i,t-1}
+ \beta_{3}\, SocialExp_{i,t-1} \\[0.4em]
&+ \beta_{4}\, Pandemic_{t}
+ \beta_{5}\, (HealthExp_{i,t-1} \times Pandemic_{t})
+ \beta_{6}\, (SocialExp_{i,t-1} \times Pandemic_{t}) \\[0.4em]
&+ \beta_{7}\, GDP_{i,t}
+ \beta_{8}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-1}
+ u_{2i}\, SocialExp_{i,t-1}
+ \varepsilon_{i,t}
...
```{r}
# extracting the values from the summary and summing the effects
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HospBeds_{i,t-[1,3]}
+ \beta_{2}\, HealthExp_{i,t-[1,3]}
+ \beta_{3}\, SocialExp_{i,t-[1,3]} \\[0.4em]
&+ \beta_{4}\, Pandemic_{t}
+ \beta_{5}\, (HealthExp_{i,t-[1,3]} \times Pandemic_{t})
+ \beta_{6}\, (SocialExp_{i,t-[1,3]} \times Pandemic_{t}) \\[0.4em]
&+ \beta_{7}\, GDP_{i,t}
+ \beta_{8}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-[1,3]}
...
Mortality_{i,t} =\;&
\beta_{0}
+ \beta_{1}\, HospBeds_{i,t-[1,5]}
+ \beta_{2}\, HealthExp_{i,t-[1,5]}
+ \beta_{3}\, SocialExp_{i,t-[1,5]} \\[0.4em]
&+ \beta_{4}\, Pandemic_{t}
+ \beta_{5}\, (HealthExp_{i,t-[1,5]} \times Pandemic_{t})
+ \beta_{6}\, (SocialExp_{i,t-[1,5]} \times Pandemic_{t}) \\[0.4em]
&+ \beta_{7}\, GDP_{i,t}
+ \beta_{8}\, Unemp_{i,t} \\[0.4em]
&+ u_{0i}
+ u_{1i}\, HealthExp_{i,t-[1,5]}
...
Third, we compare the 3 model's AIC to determine which fits best the data. 
