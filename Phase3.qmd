...
Firstly, the variance of the random intercept `REF_AREA (Intercept)` is 7790, which is much higher than the residual variance (`Residual`) at 1371. This suggests important differences between baseline avoidable mortality within countries. The standard deviation is 88.26, suggesting a deviation of about 88 deaths per 100,000 between countries. 

Second, we will interpret each coefficient : 
( if |t| > 1.96 (for \alpha = 5%), the coefficient is statistically significant)

All coefficients are all statistically significant.

- Intercept : The baseline number of avoidable deaths per 100,000 when all other predictors are 0 is 485.7.
- `healthExp_lag1` : An increase in health expenditure by 1% of GDP last year is associated with 10 fewer deaths per 100,000. 
- `hospBeds_lag1` : The addition of 1 bed per 1,000 inhabitants last year is associated with  fewer 10.2 deaths per 100,000.
- `socialExp_lag1`: The increase in social expenditure by 1% of GDP last year is associated with 3.17 fewer deaths per 100,000 this year.
- `OBS_VALUE_gdp` : An increase in GDP by 1 dollar (constant in 2015$US) reduces avoidable mortality by 0.004084 deaths per 100,000.
- `OBS_VALUE_unemp` : increase in unemployment by 1% of GDP is associated with 1.085 more deaths per 100,000. 
...
Third, the AIC is 8034.7. The residuals are centered around 0 (median : -0.1280). Looking at Q1 and Q3, the residuals seem mostly symmetric and close to the median. Max : 7.3821 indicates the presence of at least one outlier. 

In conclusion, increases in social and particularly health expenditure reduce the avoidable mortality rate in the short-term. As expected, GDP reduces mortality while unemployment increases it. 
...
```
*Interpretation of the results* 
First, the variance of the random intercept is still much higher than the residual variance (6699 against 1209). The standard deviation is 81.85, suggestinf a deviation of about 81 deaths per 100,000 between countries. This analysis is similar to the short-run regression results. 

Second, we will interpret each coefficient : 

All coefficients are all statistically significant except for `OBS_VALUE_unemp` (t = 1.51)

- Intercept : The baseline number of avoidable deaths per 100,000 when all other predictors are 0 is 500.6.
- `healthExp_ma3` : An increase in health expenditure by 1% of GDP over the past 3 years is associated with 9.13 fewer deaths per 100,000. 
- `hospBeds_ma3` : The addition of 1 bed per 1,000 inhabitants over the past 3 years is associated with an increase of deaths per 100,000 by 9.04.
- `socialExp_ma3`: The increase in social expenditure by 1% of GDP over the past 3 years is associated with 5.45 fewer deaths per 100,000 this year.
- `OBS_VALUE_gdp` : An increase in GDP by 1 dollar (constant in 2015$US) reduces avoidable mortality by  0.0034 deaths per 100,000.
- `OBS_VALUE_unemp` : increase in unemployment by 1% of GDP is associated with 9.5 more deaths per 100,000.

Third, the AIC is 7065.8 (lower than for the short-run), indicating better fit for medium-run effects.

In conclusion, health and social expenditure reduce mortality over 3 years, suggesting that the effects accumulate. Unexpectedly, there is a positive association between the number of hospital beds and the avoidable mortality rate.  
...
```
*Interpretation of the results*
First, the variance of the random intercept is still higher than the residual variance (5991 against 1140), although less so than for the two previous models. The standard deviation is 77.40 , suggesting a deviation of about 77 deaths per 100,000 between countries. We can notice that in the longer-run, the deviation between countries becomes smaller. 

Second, we will interpret each coefficient : 

All coefficients are all statistically significant except for `OBS_VALUE_unemp` (t = 0.408)

- Intercept : The baseline number of avoidable deaths per 100,000 when all other predictors are 0 is 487.
- `healthExp_ma5` : An increase in health expenditure by 1% of GDP over the past 5 years is associated with 7.97 fewer deaths per 100,000. 
- `hospBeds_ma5` : The addition of 1 bed per 1,000 inhabitants over the past 5 years is associated with an increase of deaths per 100,000 by 8.89.
- `socialExp_ma5`: The increase in social expenditure by 1% of GDP over the past 5 years is associated with 5.7 fewer deaths per 100,000 this year.
- `OBS_VALUE_gdp` : An increase in GDP by 1 dollar (constant in 2015$US) reduces avoidable mortality by  0.003 deaths per 100,000.
- `OBS_VALUE_unemp` : increase in unemployment by 1% of GDP is associated with 0.26 more deaths per 100,000.

Third, the AIC is 6167.7 (lower than for the short-run and medium-run models), indicating better fit for long-run effects.
...
The first thing we notice is that in all 3 models all variables are significant at the alpha = 5% level, except `OBS_VALUE_unemp`, they have a t value superior to 1.96 in absolute value. It means that whether it is in the short, medium or long-run they can impact the level of mortality. 
...
Over a 3-year horizon, cumulative government spending begins to show efficiency: health expenditure reduces mortality by -9.13, social expenditure by -5.45, and hospital beds now show a positive association with mortality (+9.04). This could reflect important investments in the number of beds per 1,000 inhabitants rather than a causal effect on mortality. This highlights that medium-run effects are partly shaped by policy planning and investment patterns, rather than short-run decisions. 
...
Health expenditure reduces deaths by -7.97 and social expenditure by -5.70 over five years, showing a persistent effect, although slightly smaller than medium-run averages for health expenditure. Hospital beds remain positively associated with mortality (+8.90). Moreover, GDP consistently reduces mortality across all horizons, while unemployment matters primarily in the short run. 
...
- For the short-run model : 8034.7
- For the medium-run model : 7065.8
- For the long-run model : 6167.7
